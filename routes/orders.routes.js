...
router.get('/', authMiddleware, async (req, res) => { // AuthMiddleware will set req.user
  try {
    ...
    console.log('Orders request - User Role:', userRole, 'User ID:', userId); // Debug log
    ...
    if (userRole === 'admin') {
      ...
      console.log('Admin user - fetching all orders'); // Debug log
      // No additional WHERE clause needed
    } else if (userRole === 'supplier') {
      // Suppliers can only view orders where the supplier_id matches their user_id
      console.log('Supplier user - filtering orders by user_id:', userId); // Debug log
    ...
    console.log('Query parameters:', queryParams); // Debug log

    const [rows] = await db.execute(query, queryParams);

    console.log('Query result - Number of orders found:', rows.length); // Debug log
    ...
    const formattedRows = rows.map(order => ({
      ...
      supplier_id: order.supplier_id, // Include supplier_id for frontend
    ...
    console.log('Formatted rows to send:', formattedRows.length); // Debug log
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // NEW: Auto-generate invoice when supplier confirms order
    if (userRole === 'supplier' && status === 'Confirmed') {
      try {
        ...
        if (existingInvoice.length === 0) {
          ...
          const due_date = new Date(existingOrder.delivery_date);
          due_date.setDate(due_date.getDate() + 15); // 15 days after delivery date
          
          const amount = existingOrder.value;

          await db.execute(
            `INSERT INTO invoice (
              order_id, supplier_id, invoice_date, due_date, amount, status
            ) VALUES (?, ?, ?, ?, ?, ?)`,
            [id, existingOrder.supplier_id, invoice_date, formatDateForFrontend(due_date), amount, 'pending']
          );

          console.log('Invoice auto-generated for confirmed order:', id);
        }
      } catch (invoiceErr) {
        console.error('Failed to auto-generate invoice:', invoiceErr);
        // Don't break the order update if invoice generation fails
      }
    }
