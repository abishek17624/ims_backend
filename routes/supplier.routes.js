...
// router.put('/change-password', authMiddleware, authorizeRoles('supplier'), async (req, res) => { // <--- ADDED authorizeRoles('SUPPLIER')
//   const userId = req.user.id; // User ID from the authenticated token
//   // userRole is implicitly 'supplier' due to authorizeRoles('SUPPLIER')
//   const { currentPassword, newPassword } = req.body;

//   // Basic validation
//   if (!currentPassword || !newPassword) {
//     return res.status(400).json({ success: 0, message: "Current password and new password are required." });
//   }
//   if (newPassword.length < 8) { // Align with frontend password strength policy
//     return res.status(400).json({ success: 0, message: "New password must be at least 8 characters long." });
//   }

//   try {
//     // 1. Fetch user from 'users' table to verify current password
//     const [userRows] = await db.execute('SELECT id, password FROM users WHERE id = ?', [userId]);
...
//     // 5. Update password in the 'users' table
//     await db.execute(
//       'UPDATE users SET password = ? WHERE id = ?',
//       [hashedPassword, userId]
//     );
...
//     res.status(200).json({ success: 1, message: "Password updated successfully." });
//   } catch (err) {
//     console.error("Change password error for supplier:", err);
//     res.status(500).json({ success: 0, message: 'Server error during password update.', error: err.message });
//   }
// });
